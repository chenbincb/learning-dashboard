        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-all hover:shadow-md flex flex-col h-full">
            <div className="bg-slate-50/50 dark:bg-slate-800/50 px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg">
                    样本量: {data.history.examCount} : {data.current.examCount}
                </span>
            </div>
            ...
            <div className="p-6 flex-1 flex flex-col justify-between space-y-6">
                ...
                <div className="relative flex flex-col md:flex-row items-center justify-between gap-6 md:gap-4">
                    {/* 19班 - 过去 */}
                    <div className="w-full md:flex-1 text-center p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 relative">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">19班时期均值</p>
                        ...
                        <div className="text-[10px] text-slate-400 mt-2 font-sans">
                            {data.history.sumScore || 0} <span className="opacity-60">/ {data.history.sumFullScore || 0}</span>
                        </div>
                        <div className="text-[10px] text-slate-500 mt-1 font-sans font-bold"># {avgRank} (均值)</div>
                    </div>

                    {/* 变化指示器 */}
                    <div className="shrink-0 flex flex-col items-center rotate-90 md:rotate-0">
                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110 ${isRankImproved ? 'bg-emerald-500 shadow-emerald-500/20' : 'bg-rose-500 shadow-rose-500/20'}`}>
                            {isRankImproved ? (
                    ...
                    {/* 当前班级 - 现在 */}
                    <div className="w-full md:flex-1 text-center p-4 rounded-2xl bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100/50 dark:border-indigo-800/30 relative">
                        ...
                        <p className="text-[10px] font-bold text-indigo-500 dark:text-indigo-400 uppercase mb-2">{data.current.class}时期均值</p>
                        ...
                        <div className="text-[10px] text-indigo-400 dark:text-indigo-300 mt-2 font-sans">
                            {data.current.sumScore || 0} <span className="opacity-60">/ {data.current.sumFullScore || 0}</span>
                        </div>
                        <div className="text-[10px] text-indigo-600 dark:text-indigo-400 mt-1 font-sans font-bold"># {data.current.gradeRank} (均值)</div>
                    </div>
                </div>

                {/* 关键位移大卡 */}
                <div className={`p-4 rounded-2xl border-2 border-dashed flex flex-col sm:flex-row items-center sm:items-baseline justify-between gap-3 ${isRankImproved ? 'bg-emerald-50/30 border-emerald-200/50 dark:bg-emerald-950/10' : 'bg-rose-50/30 border-rose-200/50 dark:bg-rose-950/10'}`}>
                    <div className="flex items-baseline gap-2">
                        <span className={`text-3xl md:text-4xl font-black tracking-tighter ${isRankImproved ? 'text-emerald-600' : 'text-rose-600'}`}>
                            {isRankImproved ? '+' : '-'}{absRankChange}
                        </span>
                        <span className="text-[10px] md:text-xs font-bold text-slate-500 uppercase whitespace-nowrap">均值位次波动</span>
                    </div>
                    <div className="w-full sm:max-w-[180px] text-center sm:text-right">
                ...
                {/* AI 结论导語 */}
                <div className="pt-2">
                    <p className="text-xs text-slate-500 leading-relaxed bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl italic text-center whitespace-pre-line">
