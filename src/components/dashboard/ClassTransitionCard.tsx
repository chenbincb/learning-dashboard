    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-all hover:shadow-md flex flex-col h-full">
            ...
            <div className="p-6 flex-1 flex flex-col justify-between space-y-6">
                ...
                {/* 关键位移大卡 */}
                <div className={`p-4 rounded-2xl border-2 border-dashed flex flex-col sm:flex-row items-center sm:items-baseline justify-between gap-3 ${isRankImproved ? 'bg-emerald-50/30 border-emerald-200/50 dark:bg-emerald-950/10' : 'bg-amber-50/30 border-amber-200/50 dark:bg-amber-950/10'}`}>
                    <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-[10px] md:text-xs font-bold text-slate-500 uppercase whitespace-nowrap">位次波动</span>
                    </div>
                ...
                {/* AI 结论导語 */}
                <div className="pt-2">
                    <p className="text-xs text-slate-500 leading-relaxed bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl italic text-center">
                        {`通过横向对比发现，你在 ${data.current.class} 的发挥效率较 19 班时期${isRankImproved ? '有显著提升' : '稍显滞后'}。建议保持当前的复习节奏，继续深化知识点的颗粒度。`}
                    </p>
                </div>
            </div>
        </div>
