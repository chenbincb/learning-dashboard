            ...
            <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2 shrink-0 whitespace-nowrap">
                    ...
                    <div className="flex gap-1.5 font-sans">
                        {classRank && (
                            <span className="text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-1 py-0.5 rounded font-bold flex items-center group cursor-default shrink-0">
                                班{classRank}
                                {renderTrend(classRank, prevClassRank)}
                            </span>
                        )}
                        {gradeRank && (
                            <span className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-1 py-0.5 rounded font-bold flex items-center group cursor-default shrink-0">
                                年{gradeRank}
                                {renderTrend(gradeRank, prevGradeRank)}
                            </span>
                        )}
                        {artsScienceRank && (
                            <span className="text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/30 px-1 py-0.5 rounded font-bold flex items-center group cursor-default shrink-0">
                                组{artsScienceRank}
                            </span>
                        )}
                        {scaledScore && scaledScore > 0 && scaledScore !== score && (
                            <span className="text-xs text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/30 px-1 py-0.5 rounded font-bold flex items-center group cursor-default shrink-0">
                                原{score}
                            </span>
                        )}
                        {classAvg && (
                            <span className="text-xs text-blue-600 dark:text-blue-400 px-1 py-0.5 font-bold shrink-0">
                                均:{classAvg}
                            </span>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-2">
                    <div className="flex flex-col items-end">
                        <div className="flex items-baseline gap-1">
                            <span className="text-slate-900 dark:text-slate-100 font-black text-2xl tracking-tighter">
                                {finalScore}
                            </span>
