...
### 1. Zeabur (推荐 - 简单且支持持久化)
Zeabur 是部署此类项目的理想选择，因为它支持 **Persistent Volume (持久化卷)**。

**详细步骤：**
1.  **连接 GitHub**：在 Zeabur 控制台关联你的代码仓库。
2.  **自动识别**：Zeabur 会自动识别 Next.js 项目并开始构建。
3.  **配置持久化卷 (关键)**：
    -   在服务页面点击 **"Resources" (资源)** 标签。
    -   找到 **"Volumes" (卷)** 部分，点击 **"Add Volume"**。
    -   **Mount Path (挂载路径)** 填写：`/app/data`。
    -   这样 `data/scores.db` 就会被保存在持久存储中，重新部署也不会丢失数据。
4.  **导入初始数据**：
    -   部署完成后，你可以通过项目自带的“数据导入”功能上传 JSON 文件，数据将持久化到挂载的卷中。

- 优点：一键部署，自动配置 SSL (HTTPS)，支持数据库持久化。
